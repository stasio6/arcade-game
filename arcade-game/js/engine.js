...
var Engine = (function(global) {
     ...
     */
    function update(dt, last) {
        updateEntities(dt, last);
        checkCollisions();
    }
     ...
     * render methods.
     */
    function updateEntities(dt, last) {
        allEnemies.forEach(function(enemy) {
            enemy.update(dt);
        });
        var nowyCzas=new Date().getTime();
        allDrewno.forEach(function(drewno) {
            drewno.update(last, nowyCzas);
        });
        player.update(last, nowyCzas);
    }
    ...
    function render() {
        ...
        if (timeLeft<0)
            lose();
        ctx.fillText("TIME LEFT: "+Math.floor(timeLeft), 400, 35);
        ctx.fillStyle = "red";
